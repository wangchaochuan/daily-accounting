  ...
  <view class="container">
    <scroll-view class="books" scroll-y>
      <view
        ...
        v-for="book in books"
        :key="book._id"
        :class="{ active: book._id === activeBook }"
      >
        <view class="title">
          <view class="name">{{ book.name }}</view>
          <view class="type">{{
            book.type === "private" ? "私人账本" : "共享账本"
          }}</view>
          <view class="selected" v-show="book._id === activeBook"
            >默认账本</view
          >
        </view>
        <view class="body">
        ...
        <view class="footer">
          <view
            class="delete"
            v-show="book._id !== activeBook && books.length > 1"
          >
    ...
    <view class="footer">
      <u-button type="primary" shape="circle" @click="createBook"
        >创建新的账本</u-button
      >
    </view>
    <u-modal
...
onShareAppMessage((params) => {
  if (params.from === "button") {
    ...
    return {
      title: `${userName.value}邀请您使用青牛记账小程序，并加入共享账本:${bookName}`,
