      ...
      <view class="header">
        ...
        <view class="expend">
          <view class="label">
            ...
            <u-icon
              :name="showExpend ? 'eye-off' : 'eye-fill'"
              :size="16"
              @click="showExpend = !showExpend"
            ></u-icon>
          </view>
          <view class="value">
            <u-text
              :mode="showExpend ? 'price' : 'text'"
              type="success"
              bold
              :size="16"
              :text="showExpend ? totalExpend : '****'"
            ></u-text>
          </view>
        </view>
        <view class="income">
          <view class="label">
            ...
            <u-icon
              :name="showIncome ? 'eye-off' : 'eye-fill'"
              :size="16"
          ...
          <view class="value">
            <u-text
              :mode="showIncome ? 'price' : 'text'"
              type="success"
              bold
              :size="16"
              :text="showIncome ? totalIncome : '****'"
            ></u-text>
          </view>
        </view>
      </view>
      ...
      <view class="classify-list">
        ...
        <view class="content" v-else>
          ...
          <view v-for="item in classifyData" :key="item.classify" class="row">
            ...
            <view class="perenct">
              <u-text
                :text="getPerenct(item.totalAmount)"
                :size="16"
                :line-height="24"
              ></u-text>
            </view>
...
const getRecordData = async () => {
  if (year.value && month.value) {
    ...
    const response = await db
      .collection("account-item")
      .where(condition)
      .groupBy("classify")
      .groupField("sum(amount) as totalAmount")
...
onShareAppMessage((params) => {
  return {
    title: `${userName.value}邀请您使用青牛记账小程序`,
    path: `/pages/home/home`,
    imageUrl:
      "https://mp-ab46a2a7-d28c-4b66-829a-a99b259ac741.cdn.bspapp.com/cloudstorage/004466e7-7591-4fdc-a3fc-d598d2c708d4.",
  };
});
</script>
