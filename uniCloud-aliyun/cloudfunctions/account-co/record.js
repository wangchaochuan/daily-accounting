...
async function creatRecord(params) {
  ...
  const data = { ...params, updateTimestamp: new Date().valueOf() };
  const response = await accountCollection.add(data);
  return {
    errCode: 0,
    errMsg: "新增成功",
    data: response,
...
async function getRecordList(params) {
  const { type, bookId, startDay, endDay } = params;
  if (!bookId) {
    return {
      errCode: 10001,
      errMsg: "参数错误: 缺少必要的参数",
...
async function getRecordListByClassify(params) {
  const { type, bookId, classify, startDay, endDay } = params;
  if (!bookId) {
    return {
      errCode: 10001,
      errMsg: "参数错误: 缺少必要的参数",
...
async function updateRecord(id, params) {
  ...
  const data = { ...params, updateTimestamp: new Date().valueOf() };
  const response = await accountCollection.doc(id).update(data);
  return {
    errCode: 0,
    errMsg: "更新成功",
    data: response,
