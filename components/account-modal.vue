<template>
	<u-popup :show="visible" :closeable="true" :closeOnClickOverlay="false" :round="24" @close="close" @open="open">
		<view class="content">
			...
			<view class="type">
				<u-radio-group v-model="type" name="type" @change="changeType" :disabled="mode==='edit'">
			...
			<u-form labelPosition="left" :model="model" :rules="rules" ref="formRef">
				<u-form-item label="日期" prop="date" required @click="showDatePicker=true">
	...
	const confirm = async () => {
		...
		if (props.mode === "create") {
			const data = {
				...
				createTime: dayjs(model.date).format('YYYY-MM-DD HH:mm'),
				updater: userId.value,
				updateTime: dayjs(model.date).format('YYYY-MM-DD HH:mm'),
			}
